...
export default function HomePage() {
  ...
  const executiveCockpit = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KPICard title="GMV (MTD)" value={kpiVal('GMV (MTD)', 'Rp 4.8T')} status="neutral" />
        <KPICard title="Active Merchants" value={kpiVal('Active Merchants', '124K')} status="neutral" />
        <KPICard title="Fraud Rate" value={kpiVal('Fraud Rate', '0.08%')} status="neutral" />
        <KPICard title="Order Fulfillment" value={kpiVal('Order Fulfillment', '96.2%')} status="neutral" />
      </div>
  ...
  const domainTab1 = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <KPICard title="Avg Order Value" value={kpiVal('Avg Order Value', 'Rp 287K')} />
        <KPICard title="Repeat Purchase" value={kpiVal('Repeat Purchase', '42%')} />
        <KPICard title="Cart Abandonment" value={kpiVal('Cart Abandonment', '68%')} />
      </div>
      <Chart
  ...
  const domainTab2 = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        />
        <ActionMemo
          persona={{ name: 'Andi Wijaya', role: 'VP Marketplace Ops' }}
          context={{}}
          onGenerate={async () => {
            const memos = [
              {
                ...
                actions: ['Investigate payment fraud spike in Jabodetabek', 'Onboard 500 new SME merchants in Tier-2 cities', 'Launch same-day delivery for Jakarta Premium'],
              },
              {
    ...
    <div className="h-[600px]">
      <AskAI
        ...
        sampleQuestions={[
          ...
          'Show fraud patterns by payment method',
          'What is the LTV by customer acquisition channel?',
        ]}
        mode="sql"
  ...
  const tabs = [
    ...
    { id: 'domain-2', label: 'Fraud & Risk', icon: '⚡', content: domainTab2 },
